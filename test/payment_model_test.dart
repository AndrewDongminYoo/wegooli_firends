// 📦 Package imports:
import 'package:test/test.dart';

// 🌎 Project imports:
import 'package:wegooli_friends/lib.dart';

// tests for PaymentModel
void main() {
  const PaymentModel? instance = /* PaymentModel(...) */ null;
  // TODO add properties to the entity

  group(PaymentModel, () {
    // String paymentKey
    test('to test the property `paymentKey`', () async {
      // TODO
    });

    // String status
    test('to test the property `status`', () async {
      // TODO
    });

    // String lastTransactionKey
    test('to test the property `lastTransactionKey`', () async {
      // TODO
    });

    // String orderId
    test('to test the property `orderId`', () async {
      // TODO
    });

    // String orderName
    test('to test the property `orderName`', () async {
      // TODO
    });

    // String requestedAt
    test('to test the property `requestedAt`', () async {
      // TODO
    });

    // String approvedAt
    test('to test the property `approvedAt`', () async {
      // TODO
    });

    // String cancelReason
    test('to test the property `cancelReason`', () async {
      // TODO
    });

    // String canceledAt
    test('to test the property `canceledAt`', () async {
      // TODO
    });

    // int cancelAmount
    test('to test the property `cancelAmount`', () async {
      // TODO
    });

    // String type
    test('to test the property `type`', () async {
      // TODO
    });

    // String currency
    test('to test the property `currency`', () async {
      // TODO
    });

    // int totalAmount
    test('to test the property `totalAmount`', () async {
      // TODO
    });

    // int balanceAmount
    test('to test the property `balanceAmount`', () async {
      // TODO
    });

    // int suppliedAmount
    test('to test the property `suppliedAmount`', () async {
      // TODO
    });

    // int vat
    test('to test the property `vat`', () async {
      // TODO
    });

    // int taxFreeAmount
    test('to test the property `taxFreeAmount`', () async {
      // TODO
    });

    // int taxExemptionAmount
    test('to test the property `taxExemptionAmount`', () async {
      // TODO
    });

    // String method
    test('to test the property `method`', () async {
      // TODO
    });

    // String accountId
    test('to test the property `accountId`', () async {
      // TODO
    });
  });
}
